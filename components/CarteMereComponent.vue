                        ...
                        <tbody>
                            <tr v-for="produit in produits" :key="produit.id"
                                ...
                                <th scope="row" class="py-1 px-2  text-gray-900 dark:text-white">
                                    {{ produit.name }}<img v-if="produit.img != ''"
                                        :src="require(`~/assets/images/${produit.img}`)" alt=""
                                        class="object-fill mx-auto w-16 hover:w-48" />
                                </th>
                                <td class="py-1 px-6 text-lg bg-gray-200 font-bold">{{ produit.price }}€</td>

                                <td class="py-1 px-2 ">{{ produit.format }}</td>
                                <td class="py-1 bg-gray-200 px-2">{{ produit.socket }}</td>
...
export default {
  ...
  methods: {
    async fetch(panierId) {
      this.loading = true;
      this.produits = await this.$axios.$get(`produits/carteMeres/`);
      const panier = await this.$axios.$get("produits/panier/" + panierId + "/")
      if (panier.processeur !== null) {
        this.produits = await this.produits.filter(
          (produit) => produit.socket === panier.processeur_detail.socket
        );
      }
      if (panier.boitier !== null) {
        if (panier.boitier_detail.format === "mAtx") {
          this.produits = await this.produits.filter(
            (produit) => produit.format === "mAtx"
          );
        }
      }
      this.loading = false;
    ...
    ajoutPanier(produit) {
      this.boitier = { carteMere: produit.id };
      this.alimChoisi = 0;
      this.$emit("ajout", {
        x: this.boitier,
        y: this.alimChoisi,
