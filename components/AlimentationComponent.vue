        ...
        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
          ...
          <tbody>
            <tr v-for="produit in produits" :key="produit.id"
              ...
              <td class="py-1 px-2 text-xl bg-gray-200 font-bold">{{ produit.price }}€</td>
              ...
              <td class="py-1 px-2 ">
                ...
                <span v-else class="rounded-full h-8 w-8 flex bg-red-600 m-auto"> </span>
              </td>
...
export default {
  ...
  async fetch() {
    this.loading = true;
    // this.panier = await this.$axios.$get(`produits/panier/1`);
    this.produits = await this.$axios.$get(`produits/alimentations/`);
    this.loading = false;
  ...
  methods: {
    ...
    ajoutPanier(produit) {
      this.boitier = { alimentation: produit.id };
      this.alimChoisi = 1;
      this.$emit("ajout", {
        x: this.boitier,
        y: this.alimChoisi,
